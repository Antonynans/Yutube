...
export const Header: React.FC<Itoogle> = ({toggleSidebar}) => {
  ...
  return (
    <div className='bg-white flex items-center justify-between fixed top-0 left-0 right-0 z-20'>
      ...
      >
        <FaBars className='cursor-pointer w-10 h-10 p-2 md:block hidden' />
        <div className=''>
      <Link href='/'>
        <a className='cursor-pointer flex items-center bg-white pt-5 pb-2 md:pl-6 text-2xl font-bold tracking-tighter relative'>
      ...
      <div className='relative flex items-center justify-center '>
        <input type='text' placeholder='Search' className='rounded-md md:bg-gray-200 bg-white focus:outline-none  lg:w-[35rem] md:w-[20rem] w-full py-2 px-4' 
        value={inputValue}
      ...
      <div className='shadow-lg ml-5 p-2 rounded-full hidden md:block cursor-pointer'>
      <BsMicFill className='text-2xl   ' />
      </div>
      </div>
      <div className='flex items-center justify-between bg-white gap-8 md:mr-8 mr-4'>
        <BsCameraVideo className='text-3xl bg-white cursor-pointer hidden md:block' />
